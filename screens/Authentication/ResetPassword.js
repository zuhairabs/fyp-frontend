...
import { View, Text, Linking, ActivityIndicator, ToastAndroid } from 'react-native'
...
const PhoneNumber = ({ phone, setPhone, checkPhoneRegistered }) => {
    ...
    return <>
        ...
        <View style={styles.buttonArea}>
            <TouchableOpacity style={buttons.roundedPrimaryButton} onPress={() => { validatePhone() }}>
    ...
    const changePasswordRequest = (password) => {
        ...
        try {
            fetch('https://shopout.herokuapp.com/user/password/reset', {
                ...
                .then(res => {
                    if (res.status === 200)
                        navigation.navigate("Success", { text: "Your password has been changed successfully" })
                    else {
                        setLoading(false)
        ...
        <View style={styles.screenContainer}>
            ...
            <ScrollView style={styles.container}>
                ...
                <View style={styles.contentContainer} contentContainerStyle={{ justifyContent: "center", alignItems: "center" }}>
                    ...
                    {
                        !credentialsEntered
                            ? <PhoneNumber phone={phone} setPhone={setPhone} checkPhoneRegistered={checkPhoneRegistered} />
                            : (
                                verified
                                    ? <ChangePassword changePasswordRequest={changePasswordRequest} loadingModal={loadingModal} />
                                    : <OTPVerification otp={otp} setOtp={setOtp} verifyOtpRequest={verifyOtpRequest} loadingModal={loadingModal} />
                            )
                    }
