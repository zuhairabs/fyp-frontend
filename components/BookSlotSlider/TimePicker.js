...
const Dropdown = ({list, onSelect, visible = false, selected, title}) => {
  ...
  return (
    <View style={{...styles.dropdown, ...getDropdownHeight()}}>
      ...
      <View style={styles.timeSelector}>
        ...
        <View style={styles.timeBox}>
          <TouchableOpacity
            ...
            <Text style={styles.time}>{parseTimeTo12Hour(selected)}</Text>
          </TouchableOpacity>
        </View>
      </View>
      {expanded && (
...
export default function TimePicker({
  ...
  const createTimeList = (startTime, endTime) => {
    ...
    let res = [];
    let min = new Date(startTime).getTime(),
  ...
  useEffect(() => {
    ...
    let open = stringToTime(start),
      close = stringToTime(storeEnd);
  ...
  const onChangeEnd = (val) => {
    ...
    if (stringToTime(start) >= stringToTime(val)) {
      let adjustedStartTime = timeToString(
        stringToTime(val) - serviceTime * MINUTE,
      );
  ...
  return (
    <View style={styles.timeContainer}>
      <Text style={styles.timeHeading}>Please choose a slot</Text>
      <TimeSelector
        text={'Check In'}
